...
func verifyCommandLineArgs(args []string) (bundlePath string, manifestPath string, err error) {
	if len(args) == 1 {
		return "duffle.json", "./cnab/app/kab/manifest.yaml", nil
	}
	if len(args) != 3 {
		return "", "", errors.New("usage: ./list-images <path/to/duffle.json> </path/to/riff-kab-manifest.yaml>")
	}
